/*
 * 사용자 정보 처리 모듈
 * 데이터베이스 관련 객체들을 req.app.post('database')로 참조
 *
 */
...
var listuser4pug = function(req, res) {
	console.log('user(user2.js) 모듈 안에 있는 listuser 호출됨.');
	...
	if (database.db) {
		...
		database.UserModel.findAll(function(err, results) {
			...
			if (results) {
				...
				// 뷰 템플릿을 이용하여 렌더링한 후 전송
				var context = {results : results};
				req.app.render('listuser2', context, function(err, html) {
					if (err) {
						console.error('뷰 렌더링 중 오류 발생 : ' + err.stack);						
						res.write('<h2>뷰 렌더링 중 오류 발생</h2>');
						res.write('<p>' + err.stack + '</p>');
...
module.exports.listuser4pug = listuser4pug;
